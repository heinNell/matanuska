// NOTE: Converted from TypeScript-flavored JS to pure JS to fix parsing errors in a .mjs context.
// All runtime logic preserved. Generated output still contains TypeScript where intended.

/* global console */

// Removed: import type { ... } (TS-only)
import eslintPlugin from '@typescript-eslint/eslint-plugin';
import fs from 'node:fs';
import path from 'node:path';
import prettier from 'prettier';
import { PACKAGES_ESLINT_PLUGIN, PRETTIER_CONFIG_PATH } from './paths.mjs';

// Simple color helpers
const chalk = {
  blueBright: val => `\x1B[94m${val}\x1B[39m`,
  dim: val => `\x1B[2m${val}\x1B[22m`,
  gray: val => `\x1B[90m${val}\x1B[39m`,
  green: val => `\x1B[32m${val}\x1B[39m`,
  red: val => `\x1B[31m${val}\x1B[39m`,
};
...
const CLASSIC_EXTENDS = EXTENDS_MODULES.map(mod => mod.packageRelativePath);
...
const allRuleEntries = Object.entries(eslintPlugin.rules).sort((a, b) =>
  a[0].localeCompare(b[0]),
);

/**
 * reducer builds the rules object
 * @param {object} config
 * @param {[string, any]} entry
 * @param {object} settings
 * @returns {object}
 */
function reducer(config, [key, value], settings = {}) {
...
async function writeConfig({ description, getConfig, name }) {
  ...
  if (config.extends) {
    for (const extendPath of config.extends) {
      const found = EXTENDS_MODULES.find(
      ...
      flatCode.push(
        `import ${found.name} from '${found.moduleRelativePath}';`,
      );
      flatExtends.push(found.name);
    }
    flatCode.push('');
  }
...
function filterRuleEntriesTo(...recommendations) {
  return allRuleEntries.filter(([, rule]) =>
    typeof rule.meta.docs.recommended === 'object'
      ? Object.keys(rule.meta.docs.recommended).some(level =>
          recommendations.includes(level),
        )
      : recommendations.includes(rule.meta.docs.recommended),
  );
}

function createGetOptionsForLevel(level) {
  return rule =>
    typeof rule.meta.docs.recommended === 'object'
