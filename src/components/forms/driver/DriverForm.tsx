...
  const bloodTypes = useBloodTypes();
  ...
  const [formData, setFormData] = useState<DriverData>(() => {
    ...
    // Merge initialData into the copy, providing fallbacks for nested objects
    mergedData.licenseInfo = { ...mergedData.licenseInfo, ...initialData.licenseInfo };

    // Handle emergencyContact - only include if initialData has it, otherwise keep defaultFormData's version
    ...
    // Handle medicalInfo - only include if initialData has it, otherwise keep defaultFormData's version
    if (initialData.medicalInfo) {
      mergedData.medicalInfo = {
        conditions: initialData.medicalInfo.conditions || "",
        bloodType: initialData.medicalInfo.bloodType || "",
        allergies: initialData.medicalInfo.allergies || "",
      };
    }

    // Merge top-level properties
    const topLevelKeys = Object.keys(defaultFormData) as (keyof DriverData)[];
    topLevelKeys.forEach(key => {
      if (initialData[key] !== undefined && key !== 'emergencyContact' && key !== 'medicalInfo' && key !== 'licenseInfo') {
        (mergedData as any)[key] = initialData[key];
      }
    });

    // Ensure dateHired is always a string
    if (initialData.dateHired !== undefined) {
      mergedData.dateHired = initialData.dateHired;
    }
...
    if (name.includes(".")) {
      const [parent, child] = name.split(".") as [keyof DriverData, string];
      setFormData((prev) => {
