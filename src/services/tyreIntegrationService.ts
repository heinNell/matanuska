...
class TyreIntegrationService {
  ...
  private async fetchVendorQuote(
    ...
    const demoQuote: VendorPriceQuote = {
      vendorId,
  ...
  async updateTyreMileageFromTelematics(): Promise<void> {
    try {
      ...
      const tyresWithVehicles = tyresSnapshot.docs.map((docSnapshot) => ({
        id: docSnapshot.id,
        ...docSnapshot.data(),
      })) as Tyre[];
      ...
      const vehicleGroups = tyresWithVehicles.reduce(
        (acc, tyre) => {
          ...
          if (vehicleId != null && vehicleId !== '') {
            if (!(vehicleId in acc)) acc[vehicleId] = [];
  ...
  private async fetchTelematics(vehicleId: string): Promise<TelematicsUpdate | null> {
    try {
      ...
      const demoTelematicsData: TelematicsUpdate = {
        vehicleId,
  ...
  async generateJobCardFromInspection(inspection: TyreInspection): Promise<string | null> {
    try {
      ...
      const inspectionWithVehicle = inspection as TyreInspection & { vehicleId?: string; position?: string };
      
      const jobCard: AutoGeneratedJobCard = {
        id: `AUTO-${Date.now()}`,
        fleetNumber: inspectionWithVehicle.vehicleId ?? "Unknown",
        tyrePosition: inspectionWithVehicle.position ?? "Unknown",
        priority,
  ...
  private generateDescription(inspection: TyreInspection): string {
    const priority = this.calculatePriority(inspection);
    const taskType = this.determineTaskType(inspection);
    const inspectionWithVehicle = inspection as TyreInspection & { position?: string };

    let description = `${priority.toUpperCase()} PRIORITY: ${taskType} required for tyre at position ${inspectionWithVehicle.position ?? "Unknown"}.`;
